...
This is the fastest way to get IDguard PRO running, especially if you don't want to manage a Python venv, CUDA wheels, and system packages by hand. One container, built from the project's image, running the dashboard — which starts/stops the recording pipeline itself, exactly like on a bare-metal install.
...
* **Face recognition may silently run on CPU inside the container too**, for the same reason as a bare-metal install: `insightface`/`faster-whisper` pull in plain `onnxruntime` as a dependency, which can overwrite `onnxruntime-gpu`'s files in the same Python module namespace with no error at all. Check with `docker compose exec idguard python3 -c "import onnxruntime; print(onnxruntime.get_available_providers())"` — see INSTALL.md's Face Recognition section for the fix if `CUDAExecutionProvider` is missing.
