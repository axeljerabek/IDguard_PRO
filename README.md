...
IDguard PRO doesn't ask one single AI model to do everything. Instead, it hands the work between a few different ones, each doing the job it's actually good at: a YOLO model watches the cameras and decides *when* something worth recording is happening; the recording itself then captures the event, with a bit of buffer before and after; and — if you want — a vision-language model (via Ollama) looks at the finished clip afterwards and writes a plain-language description of what happened. That handoff happens automatically, on the fly, with no manual steps in between — it's a simple idea, but it means each model only has to be good at one thing.

*(And yes, a lot of this code was written with AI — see below. But someone still had to come up with how the pieces should fit together, and put in the time to actually make it work as one coherent system instead of a folder of scripts that sort of do stuff. That part was still very much a human job.)*
